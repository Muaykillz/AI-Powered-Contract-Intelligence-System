...
class TestRAGSystem(unittest.TestCase):
    ...
    def test_process_query(self, mock_call_api):
        ...
        mock_call_api.side_effect = [
            ...
            # Mock response for self_evaluate (initial evaluation)
            {'choices': [{'message': {'content': '{"evaluation_score": 0.7, "feedback": "Good initial response", "suggestions_for_improvement": ["Add more detail"]}'}}]},
            # Mock response for generate_response (improved response)
            {'choices': [{'message': {'content': '{"answer": "Improved test answer", "references": [{"document": "doc1", "relevance": "high"}], "confidence": 0.9}'}}]},
            # Mock response for self_evaluate (final evaluation)
            {'choices': [{'message': {'content': '{"evaluation_score": 0.9, "feedback": "Excellent improved response", "suggestions_for_improvement": []}'}}]},
        ]
        ...
        # Update assertions to check for the improved response
        self.assertIn('answer', result)
        self.assertIn('evaluation_score', result)
        self.assertGreater(result['evaluation_score'], 0.8)  # Expecting improved score
        self.assertEqual(result['answer'], "Improved test answer")
...
#TODO : python -m unittest unit_test/test_rag_system.py